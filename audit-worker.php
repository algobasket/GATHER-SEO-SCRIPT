...
{ 
    ...
    foreach($decode as $r)    
    {
            ...
            $spreadSheetName = "Issues Found with Website:".$spreadSheetName;      

            //print_r($spreadSheetName);echo "\r\n"; 
               
            if($spreadSheetName)
            {
                 $spreadSheetId = createSpreadSheetsFields($lk,$spreadSheetName);
                if($spreadSheetId)
                {   
                    echo "Spread Sheet Created!!";echo "\r\n";     
                   $links = auditScrap($lk,$spreadSheetId,$id,$saveToSpreadSheet = 0,$saveToDB = 1,$saveToBothSpreadAndDB = 0,$limit = 50);               
                   preTest($links);                              
                }                       
            } 
            //exit;                             
